...
export default function Plus() {
  ...
  /* Rebuilds the plan from the current record — useful right after a new
     prescription, report or weight update lands. */
  const refreshPlan = async () => {
    setRefreshing(true)
    try {
      const advice = await api.refreshAdvice()
      setData((current) => ({ ...current, advice }))
      toast('Plan rebuilt from your latest record')
    } catch (err) {
      toast(err.detail || 'Could not rebuild the plan right now')
    } finally {
      setRefreshing(false)
    }
  }
  ...
  const modelVersion = data?.advice?.[0]?.model_version
  ...
  return (
    <section id="plus" data-chapter="Care & Insights" data-nav="Plus" style={{ borderBottom: 0 }}>
      ...
      {isPremium && (
        <div className="advice-head reveal">
          <span className="muted" style={{ fontSize: 12 }}>
            {modelVersion
              ? `Generated by ${modelVersion} from your records`
              : 'Generated from your records'}
          </span>
          <button className="btn" onClick={refreshPlan} disabled={refreshing || loading}>
            {refreshing ? 'Rebuilding…' : 'Rebuild from my latest record'}
          </button>
        </div>
      )}
